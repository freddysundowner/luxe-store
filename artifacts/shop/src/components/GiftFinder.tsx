      ...
      setMessages([
        {
          role: "assistant",
          content:
            "Hi! I'm your gift advisor. Tell me who you're shopping for, their interests, and your budget — I'll find the perfect match from our collection.",
        },
      ]);
    }
    ...
    <>
      ...
      <div
        className={`fixed bottom-0 right-0 z-50 flex flex-col transition-all duration-400 ease-out
          ...
          h-[70dvh] sm:h-[520px]
          bg-[#0f0f0f] border border-[#D4AF37]/25 shadow-[0_8px_48px_rgba(0,0,0,0.6)]`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-900 shrink-0">
          ...
          <button onClick={() => setOpen(false)} className="text-zinc-600 hover:text-zinc-300 transition-colors p-1">
            <X className="w-4 h-4" />
          </button>
        </div>
                  ...
                  ? parseProductLinks(msg.content).map((part, j) =>
                      typeof part === "string" ? (
                        ...
                        <Link
                          ...
                          className="inline-block mt-1 text-[#D4AF37] underline underline-offset-2 text-xs"
                        >
                          {part.text}
                        </Link>
                      )
        ...
        <div className="px-4 py-4 border-t border-zinc-900 shrink-0">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2.5 bg-[#D4AF37] text-black hover:bg-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
