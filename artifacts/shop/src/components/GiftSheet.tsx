...
/**
 * Unified gift flow: form → pay (M-Pesa STK push) → share.
 *
 * Behavior contract:
 *  - Sender must pay before sharing. WhatsApp self-checkout was removed —
 *    gifts always start "pending" and only flip to "paid" via webhook /
 *    polling once the linked M-Pesa transaction completes.
 *  - The gift+payment are linked by `externalRef = "GIFT-<claimToken>"`.
 *    Server (`markGiftPaidForExternalRef`) keys off that prefix.
 *  - On payment success the sheet reveals share buttons that hand the link
 *    to WhatsApp / clipboard / native share — same pattern the rest of the
 *    app uses, just with a pre-composed message.
 */
export function GiftSheet({ open, product, onClose }: GiftSheetProps) {
  ...
  // so a payment-initiation retry doesn't spawn orphan pending gifts in the
  // DB (architect-flagged). Cleared when the sheet closes.
  ...
  // Poll only while the sheet is open AND we're on the pending step (architect
  // -flagged). Without the `open` gate, closing the sheet would leave the
  // query polling forever in the background.
  ...
  // Hold the gift link so we can still share even after the transaction is
  // settled. We capture it once at create time.
  ...
  const handlePay = async () => {
    if (!product) return;
    // Block double-submits while a mutation is in flight (Enter key, fast
    // double-tap, etc.) — architect-flagged.
    ...
    try {
      // 1. Create the gift (pending) the first time only. On retry we reuse
      //    the existing claimToken so the DB doesn't accumulate orphan
      //    pending gift rows — architect-flagged.
      ...
      // 2. Kick off the M-Pesa STK push, tagged with the gift token so the
      //    server can flip the gift to "paid" when the payment completes.
  ...
  return (
    <>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 z-[210] sm:max-w-md sm:left-1/2 sm:-translate-x-1/2 sm:bottom-auto sm:top-1/2 sm:-translate-y-1/2 bg-[#0a0a0a] border-t sm:border border-[#D4AF37]/20"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-900">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#D4AF37] text-sm font-light mt-0.5">{fmt.format(Number(product.price))}</p>
          </div>
        ...
        {step === "form" && (
          <div className="px-5 pt-4 pb-5 space-y-3">
            ...
            <button onClick={handleContinue}
              ...
              <Smartphone className="w-3.5 h-3.5" />Continue to pay {fmt.format(Number(product.price))}
            </button>
            <p className="text-[9px] text-zinc-700 text-center tracking-widest uppercase">Pay with M-Pesa, then share the link</p>
          </div>
        )}
        ...
        {(step === "phone" || step === "failed") && (
          <div className="px-5 pt-4 pb-5 space-y-4">
            <div className="text-center">
              ...
              <p className="text-[#D4AF37] text-2xl font-light mt-1">{fmt.format(Number(product.price))}</p>
            </div>
            {apiError && (
            ...
            <button onClick={handlePay} disabled={submitting}
              ...
              {submitting
                ...
                : <><Smartphone className="w-4 h-4" />Pay {fmt.format(Number(product.price))} via M-Pesa</>}
            </button>
          </div>
        )}
          ...
          <div className="px-5 py-8 text-center space-y-5">
            ...
            <div>
              ...
              <p className="text-zinc-500 text-xs mt-2">
                ...
                <span className="text-[#D4AF37]">{fmt.format(Number(product.price))}</span>.
              </p>
            </div>
        ...
        {/* Step: success — show gift card + share buttons */}
        {step === "success" && (
