...
import { ShoppingBag, Phone, CheckCircle2, Clock, XCircle, Hash } from "lucide-react";
    ...
    <AdminLayout title="Orders">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          {/* Desktop header */}
          <div className="hidden md:grid grid-cols-[1fr_140px_120px_160px_160px] gap-4 px-5 py-3 bg-muted/40 text-xs font-semibold uppercase tracking-wider text-muted-foreground border-b border-border">
            <span>Reference</span>
            <span>Phone</span>
          ...
          <div className="divide-y divide-border">
            {orders.map((order) => (
              <div key={order.id} className="px-4 md:px-5 py-4 md:grid md:grid-cols-[1fr_140px_120px_160px_160px] md:gap-4 md:items-center hover:bg-muted/20 transition-colors">
                {/* Mobile layout */}
                <div className="flex items-start justify-between gap-3 md:contents">
                  <div className="flex flex-col gap-0.5 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 md:hidden flex-wrap">
                      ...
                      <span className="text-xs font-semibold text-foreground">KSh {order.amount.toLocaleString()}</span>
                      {order.mpesaRef && (
                        <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  <span className="hidden md:block text-xs text-muted-foreground font-mono">
                    {order.mpesaRef ?? <span className="text-muted-foreground/40">—</span>}
                  </span>
                  <span className="hidden md:block text-xs text-muted-foreground text-right">{formatDate(order.createdAt)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <p className="text-xs text-muted-foreground text-right mt-3">Auto-refreshes every 30 s</p>
    </AdminLayout>
  );
}
